...
namespace toric
{
	...
	void ToricInterpolator::init( const Vector3 & wposA, const Vector3 & wposB, const Toric3 & t1, const Toric3 & t2 )
	{
		m_wposA = wposA;
		m_wposB = wposB;
		m_alpha[0] = t1.getAlpha()();
		m_alpha[1] = t2.getAlpha()();

		Plane
			viewPlaneA = Plane::createPlaneFromNormal(Vector3::WorldUp(), wposA),
			viewPlaneB = Plane::createPlaneFromNormal(Vector3::WorldUp(), wposB);
		...
		{ // A
			...
			y1Proj.normalize();
			y2Proj.normalize();
			m_infoA.viewAngleChange = y1Proj.directedAngle(y2Proj, Vector3::WorldUp());
			m_infoA.viewHeightChange = (Vector3::WorldUp().angle(y1) - Vector3::WorldUp().angle(y1Proj)) - (Vector3::WorldUp().angle(y2)-Vector3::WorldUp().angle(y2Proj));
		...
		{ // B
			...
			y1Proj.normalize();
			y2Proj.normalize();
			m_infoB.viewAngleChange = y1Proj.directedAngle(y2Proj, Vector3::WorldUp());
			m_infoB.viewHeightChange = (Vector3::WorldUp().angle(y1) - Vector3::WorldUp().angle(y1Proj)) - (Vector3::WorldUp().angle(y2)-Vector3::WorldUp().angle(y2Proj));
